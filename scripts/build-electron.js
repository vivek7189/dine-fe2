...
const ELECTRON_CONFIG = path.join(ROOT, 'next.config.electron.mjs');
...
const ELECTRON_HOME_PAGE = `'use client';
...
function run() {
  ...
  try {
    ...
    configBackedUp = true;
    fs.copyFileSync(ELECTRON_CONFIG, ORIGINAL_CONFIG);
    ...
    // 6. Swap root page.js with Electron redirect (opens /login instead of homepage)
    ...
    if (fs.existsSync(rootPageSrc)) {
      log('Swapping root page.js with Electron redirect...');
      fs.copyFileSync(rootPageSrc, rootPageBackup);
      fs.writeFileSync(rootPageSrc, ELECTRON_HOME_PAGE);
    ...
    if (fs.existsSync(homeClientSrc)) {
      ...
      fs.writeFileSync(homeClientSrc, '// Excluded for Electron build\nexport default function HomePageClient() { return null; }\n');
  ...
  for (let i = moved.length - 1; i >= 0; i--) {
    ...
    try {
      ...
      console.error(`[build-electron] FAILED to restore: ${src}: ${e.message}`);
    }
  }
  ...
  if (!mwExists || !vsExists || !configOk) {
    console.error('[build-electron] WARNING: Some files may not have been restored! Run: git checkout -- src/app/ src/middleware.js next.config.mjs');
  }
