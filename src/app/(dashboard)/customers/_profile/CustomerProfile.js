...
var CustomerDetail = function() {
  ...
  // Installed apps (Electron/Capacitor) are a static export where the dynamic
  // `/customers/[id]` route isn't pre-rendered, so they open the profile via the
  // static alias `/customers/view/?id=<id>`. Prefer the query id, fall back to the
  // route param (web, where the dynamic route works normally).
  var customerId = (searchParams && searchParams.get('id')) || params.id;
