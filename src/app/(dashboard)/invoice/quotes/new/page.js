...
export default function NewQuotePage() {
  ...
  useEffect(() => {
    ...
    async function loadNextNumber() {
      try {
        const data = await apiClient.getNextQuoteNumber();
        setQuoteNumber(data.nextNumber || data.quoteNumber || 'QT-001');
      } catch {
  ...
  async function handleSave(sendAfterSave = false) {
    ...
    try {
      ...
      const data = await apiClient.createInvoiceQuote(payload);
      const quoteId = data._id || data.id;

      if (sendAfterSave && quoteId) {
        try {
          await apiClient.sendInvoiceQuote(quoteId);
        } catch {
          showToast('Quote saved but failed to send', 'warning');
        }
      }

      showToast(
        sendAfterSave ? 'Quote created and sent!' : 'Quote saved as draft!',
        'success'
      );
      router.push(quoteId ? `/invoice/quotes/${quoteId}` : '/invoice/quotes');
    } catch (err) {
  ...
  const customerOptions = customers.map((c) => ({
    value: c._id || c.id,
    label: c.name || c.displayName || c.companyName || '',
  }));
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-col md:flex-row gap-8 mt-6">
            ...
            <div className="w-full md:w-80">
              ...
              {/* PDF Template selector placeholder */}
