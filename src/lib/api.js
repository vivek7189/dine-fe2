...
class ApiClient {
  ...
  async getStaffAdvances(restaurantId, options = {}) {
    ...
    return this.request(`/api/staff-advances/${restaurantId}${q ? `?${q}` : ''}`);
  }
  async createStaffAdvance(restaurantId, data) {
    return this.request(`/api/staff-advances/${restaurantId}`, { method: 'POST', body: data });
  }
  ...
  // ── Staff bonuses (HR) ───────────────────────────────────────────────
  async getStaffBonuses(restaurantId, options = {}) {
    const q = new URLSearchParams(Object.entries(options).filter(([, v]) => v != null && v !== '')).toString();
    return this.request(`/api/staff-bonuses/${restaurantId}${q ? `?${q}` : ''}`);
  }
  async createStaffBonus(restaurantId, data) {
    return this.request(`/api/staff-bonuses/${restaurantId}`, { method: 'POST', body: data });
  }
  ...
  async sendTestReport(emailOrEmails, currencySymbol, reportFrequency, reportType) {
    const body = Array.isArray(emailOrEmails)
      ? { emails: emailOrEmails }
      : { email: emailOrEmails };
    if (currencySymbol) body.currencySymbol = currencySymbol;
    if (reportFrequency) body.reportFrequency = reportFrequency;
