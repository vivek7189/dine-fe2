...
const AVATAR = { indigo: 'bg-[var(--h-brand-soft)] text-[var(--h-brand-ink)]', sky: 'bg-sky-100 text-sky-700', amber: 'bg-amber-100 text-amber-700', slate: 'bg-[var(--h-border2)] text-[var(--h-text)]', rose: 'bg-rose-100 text-rose-700', emerald: 'bg-emerald-100 text-emerald-700' };
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-[var(--h-muted)]">Your team, organised by the area they cover.</p>
            ...
            return (
              <div key={d}>
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-semibold text-[var(--h-ink2)]">{meta.label}</h3>
                  <span className="rounded-full bg-[var(--h-bsoft2)] px-2 py-0.5 text-[11px] text-[var(--h-muted)]">{byDept[d].length}</span>
                </div>
                <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
                  {byDept[d].map((s) => (
                    <div key={s.id} className="group flex items-center gap-3 rounded-xl border border-[var(--h-border)] bg-white p-3 shadow-sm">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="truncate text-sm font-semibold text-[var(--h-ink)]">{s.name}</div>
                        <div className="truncate text-xs text-[var(--h-faint)]">{s.role || meta.label}{s.shift ? ` · ${s.shift}` : ''}</div>
                        {s.phone && <div className="mt-0.5 flex items-center gap-1 text-[11px] text-[var(--h-faint)]"><FaPhone size={8} /> {s.phone}</div>}
                      </div>
                      <div className="flex flex-col gap-1 opacity-0 transition group-hover:opacity-100">
                        <button onClick={() => openEdit(s)} className="rounded p-1 text-[var(--h-faint)] hover:bg-[var(--h-hover)] hover:text-[var(--h-brand)]" aria-label="Edit"><FaPen size={11} /></button>
                        <button onClick={() => remove(s)} className="rounded p-1 text-[var(--h-faint)] hover:bg-rose-50 hover:text-rose-600" aria-label="Remove"><FaTrash size={11} /></button>
                      </div>
                    </div>
                  ))}
                </div>
