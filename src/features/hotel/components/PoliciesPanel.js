  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between gap-3">
        <p className="text-sm text-[var(--h-muted)]">Cancellation &amp; no-show rules. When a booking is cancelled past its free window (or flagged a no-show by the night audit), the fee is posted to the guest folio.</p>
      ...
      {loading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {policies.map((p) => (
            <div key={p.id} className={`group rounded-2xl border bg-white p-4 shadow-[0_1px_2px_rgba(40,33,20,0.05)] ${p.active ? 'border-[var(--h-border)]' : 'border-dashed border-[var(--h-border2)] opacity-70'}`}>
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="font-serif text-[16px] font-semibold text-[var(--h-ink)]">{p.name}</span>
                  {p.isDefault && <span className="rounded-full bg-[var(--h-brand-soft)] px-2 py-0.5 text-[10px] font-semibold text-[var(--h-brand-ink)]">Default</span>}
                </div>
                {canManage && (
              ...
              <div className="mt-3 space-y-1 text-[12px] text-[var(--h-ink2)]">
                <div className="flex justify-between"><span className="text-[var(--h-faint)]">Free until</span><span>{p.freeUntilHours}h before arrival</span></div>
                <div className="flex justify-between"><span className="text-[var(--h-faint)]">Cancellation fee</span><span className="font-medium">{chargeText(p.cancelChargeType, p.cancelChargeValue)}</span></div>
                <div className="flex justify-between"><span className="text-[var(--h-faint)]">No-show fee</span><span className="font-medium">{chargeText(p.noShowChargeType, p.noShowChargeValue)}</span></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal open={!!editing} wide title={editing?.id ? 'Edit policy' : 'New cancellation policy'} onClose={() => setEditing(null)}
        ...
        <div className="space-y-3.5">
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 text-sm text-[var(--h-text)]"><input type="checkbox" checked={form.isDefault} onChange={(e) => setForm({ ...form, isDefault: e.target.checked })} className="h-4 w-4 rounded border-[var(--h-border2)]" /> Default policy</label>
