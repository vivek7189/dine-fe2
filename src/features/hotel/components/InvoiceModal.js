      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="no-print mb-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {isDraft && <button onClick={issue} disabled={busy || loading} className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--h-brand)] px-3 py-1.5 text-[13px] font-semibold text-white hover:brightness-110 disabled:opacity-60"><FaFileInvoiceDollar size={12} /> {busy ? 'Issuing…' : 'Issue invoice'}</button>}
            <button onClick={print} disabled={loading} className="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-[13px] font-semibold text-[var(--h-ink2)] hover:bg-[var(--h-hover)] disabled:opacity-60"><FaPrint size={12} /> Print</button>
            <button onClick={onClose} className="rounded-lg bg-white/10 p-2 text-white hover:bg-white/20"><FaTimes size={13} /></button>
          </div>
        </div>

        {loading ? (
          <div className="flex items-center justify-center gap-2 rounded-2xl bg-white py-16 text-[var(--h-faint)]"><FaSpinner className="animate-spin" /> Loading…</div>
        ) : !doc ? (
          <div className="rounded-2xl bg-white py-16 text-center text-[var(--h-faint)]">No invoice available.</div>
        ) : (
          <div id="hotel-invoice" className="relative rounded-2xl bg-white p-8 text-[var(--h-ink)] shadow-2xl" style={{ fontFamily: 'var(--h-serif)' }}>
            {isDraft && <div className="no-print pointer-events-none absolute inset-0 flex items-center justify-center"><span className="rotate-[-24deg] text-[64px] font-bold text-[color-mix(in_srgb,var(--h-brand)_10%,transparent)]">DRAFT</span></div>}

            {/* header */}
            <div className="flex items-start justify-between border-b-2 border-[var(--h-ink)] pb-4">
              <div>
                <div className="text-[20px] font-bold">{doc.seller?.name}</div>
                {doc.seller?.tradeName && doc.seller.tradeName !== doc.seller.name && <div className="text-[12px] text-[var(--h-text)]">{doc.seller.tradeName}</div>}
                <div className="mt-1 text-[11.5px] leading-snug text-[var(--h-ink2)]">
              ...
              <div className="text-right">
                ...
                <div className="mt-1 text-[12px]"><span className="text-[var(--h-muted)]">No: </span><span className="font-semibold">{doc.invoiceNumber}</span></div>
                <div className="text-[12px]"><span className="text-[var(--h-muted)]">Date: </span>{fmtDateTime(doc.issuedAt)}</div>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-6 text-[12px]">
              <div>
                <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-[var(--h-muted)]">Bill to</div>
                <div className="font-semibold">{doc.buyer?.name}</div>
                {doc.buyer?.company && <div>{doc.buyer.company}</div>}
                {doc.buyer?.address && <div className="text-[var(--h-ink2)]">{doc.buyer.address}{doc.buyer.city ? `, ${doc.buyer.city}` : ''}</div>}
                {(doc.buyer?.phone || doc.buyer?.email) && <div className="text-[var(--h-ink2)]">{[doc.buyer.phone, doc.buyer.email].filter(Boolean).join('  ·  ')}</div>}
                {doc.buyer?.taxId && <div className="text-[var(--h-ink2)]">{doc.seller?.taxIdLabel || 'Tax Reg. No.'}: {doc.buyer.taxId}</div>}
              </div>
              {doc.stay && (doc.stay.checkIn || doc.stay.roomNumber) && (
                <div className="text-right">
                  <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-[var(--h-muted)]">Stay</div>
                  ...
                  {doc.stay.code && <div className="text-[var(--h-muted)]">Ref {doc.stay.code}</div>}
                </div>
              )}
            </div>
            ...
            <table className="mt-5 w-full text-[12px]">
              <thead>
                <tr className="border-b border-[var(--h-faint3)] text-left text-[10px] uppercase tracking-wide text-[var(--h-muted)]">
              ...
              <tbody>
                {(doc.charges || []).map((c, i) => (
                  <tr key={i} className="border-b border-[var(--h-bsoft)]">
                    <td className="py-1.5">{c.description}{c.category ? <span className="text-[var(--h-faint)]"> · {c.category}</span> : ''}</td>
                ...
                {doc.discountTotal ? (
                  <tr className="border-b border-[var(--h-bsoft)] text-[#8A3F31]"><td className="py-1.5">Discount</td><td /><td className="py-1.5 text-right tabular-nums">{money(doc.discountTotal)}</td></tr>
                ) : null}
              </tbody>
            </table>
            ...
            {(doc.payments || []).length > 0 && (
              <div className="mt-4 text-[11px] text-[var(--h-ink2)]">
                <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-[var(--h-muted)]">Payments</div>
                {doc.payments.map((p, i) => (
                  <div key={i} className="flex justify-between border-b border-[var(--h-bsoft)] py-1">
                    <span className="capitalize">{p.method}{p.reference ? ` · ${p.reference}` : ''}</span>
                    <span className="tabular-nums">{money(p.amount)} <span className="text-[var(--h-faint)]">{fmtDateTime(p.paidAt)}</span></span>
                  </div>
                ))}
              </div>
            )}

            <div className="mt-6 border-t border-[var(--h-bsoft)] pt-3 text-center text-[10.5px] text-[var(--h-muted)]">
...
function Row({ k, v, bold, big, sub }) {
  return (
    <div className={`flex justify-between ${sub ? 'text-[var(--h-text)]' : ''} ${bold ? 'font-semibold' : ''} ${big ? 'text-[14px]' : ''} py-0.5`}>
