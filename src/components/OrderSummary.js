...
const OrderSummary = ({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (isNative) {
        ...
        printDocument({
          html: kotContent, type: 'kot', orderId: k.orderId, restaurantId, printSettings: printSettings || {},
          orderData: {
        ...
        if (invoiceData.splitBill && invoiceData.splitBill.splits && invoiceData.splitBill.splits.length >= 2) {
          ...
          if (supportsNativeAutoPrint()) {
            ...
            allHtmlParts.forEach((html, i) => {
              setTimeout(() => printDocument({ html, type: 'bill', orderId: invoice?.id || invoice?.orderId, restaurantId, printSettings: printSettings || {}, orderData: invoiceData }), i * 600);
            });
          ...
          if (supportsNativeAutoPrint()) {
            ...
            printDocument({ html: invoiceContent, type: 'bill', orderId: invoice?.id || invoice?.orderId, restaurantId, printSettings: printSettings || {}, orderData: invoiceData });
  ...
  const handleKotBillSettle = async (method, extra = {}, oidArg = null, amountArg = null) => {
    ...
    try {
      ...
      try {
        await apiClient.verifyPayment({ orderId: oid, paymentMethod: method, amount, restaurantId, paymentStatus: 'completed' });
      } catch (_) { /* non-fatal */ }
    } catch (e) {
      console.warn('KOT+Bill settle failed:', e?.message);
    } finally {
        ...
        <div style={{
          ...
          {(() => {
            ...
            return (
              <div style={{ padding: '0 12px 8px 12px' }}>
                ...
                </div>{/* end billing panels scrollable wrapper */}
              </div>
            );
          })()}
          ...
          <div style={{
            ...
            ...(billingMode ? {
              ...
              marginTop: billingMode && isMobileEmbed ? 'auto' : '4px',
            } : {})
          }}>
          {/* Error Message */}
          {error && (
      ...
      fullDue: fullDueMode || undefined,
          ...
          <div style={{ display: 'flex', gap: billingMode ? (isMobile ? '6px' : '10px') : (isMobile ? '4px' : '8px') }}>
            ...
            >
              {processing ? (
                <>
                  <FaSpinner size={billingMode ? (isMobile ? 13 : 16) : 12} style={{ animation: 'spin 1s linear infinite' }} />
                  {t('dashboard.paymentProcessing')}
                </>
              ) : (
                <>
                  <FaCheckCircle size={billingMode ? (isMobile ? 13 : 16) : 12} />
              ...
              {canCompleteBill && isRoleAllowed(billingSettings.billAndPrintRoles) && printSettings?.enableSaveAndPrint && (
                ...
                >
                  {processing ? (
                    <><FaSpinner size={billingMode ? (isMobile ? 12 : 14) : 12} style={{ animation: 'spin 1s linear infinite' }} /> Processing...</>
                  ) : (
                    <><FaPrint size={billingMode ? (isMobile ? 12 : 14) : 12} /> Bill & Print</>
                  )}
                </button>
              )}
          </div>
